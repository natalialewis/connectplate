-- RPC for authenticated friend search: returns public profile fields only (no email).
-- Uses SECURITY DEFINER to read profiles beyond owner-only RLS; input is sanitized.
-- Matches username (handle rules) and/or first name, last name, or "first last" combined.
